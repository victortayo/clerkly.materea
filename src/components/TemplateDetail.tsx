...
export function TemplateDetail({ template, onBack, isBookmarked, bookmarkedIds, onToggleBookmark, onView }: TemplateDetailProps) {
  ...
  const showNextImage = () => {
    if(template.imageUrls) {
      setSelectedImage((prevIndex) => (prevIndex + 1) % template.imageUrls.length);
    }
  };

  const showPrevImage = () => {
    if(template.imageUrls) {
      setSelectedImage((prevIndex) => (prevIndex - 1 + template.imageUrls.length) % template.imageUrls.length);
    }
  };

  const lightbox = lightboxOpen && template.imageUrls ? createPortal(
    <div 
      className="fixed inset-0 bg-black/80 flex items-center justify-center z-[1000] cursor-pointer"
      onClick={closeLightbox}
    >
      <button 
        className="absolute top-5 right-8 text-3xl text-white bg-transparent border-none cursor-pointer"
        onClick={closeLightbox} 
        aria-label="Close"
      >&times;</button>
      <button 
        className="absolute top-1/2 -translate-y-1/2 left-5 text-3xl text-white bg-black/30 border-none cursor-pointer p-2.5"
        onClick={(e) => { e.stopPropagation(); showPrevImage(); }} 
        aria-label="Previous image"
      >&#10094;</button>
      <img
        src={template.imageUrls[selectedImage]}
        alt={`${template.title} - Image ${selectedImage + 1}`}
        className="w-[85vw] h-[85vh] object-contain cursor-default"
        onClick={(e) => e.stopPropagation()} 
      />
      <button 
        className="absolute top-1/2 -translate-y-1/2 right-5 text-3xl text-white bg-black/30 border-none cursor-pointer p-2.5"
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 animate-in fade-in slide-in-from-right duration-500">
        ...
        <div className="relative bg-indigo-950 dark:bg-slate-900 rounded-3xl p-3 sm:p-6 shadow-xl shadow-indigo-900/20 dark:shadow-none border border-indigo-800 dark:border-slate-800 mb-8 overflow-hidden">
          ...
          {template.imageUrls && template.imageUrls.length > 0 && (
            <div className="mt-6 inline-block bg-slate-900/50 rounded-2xl p-4 shadow-inner">
              <div className="flex flex-wrap justify-start gap-3">
                {template.imageUrls.map((url, index) => (
                    <button
                        key={index}
                        onClick={() => openLightbox(index)}
                        className="group relative w-24 h-24 rounded-lg overflow-hidden shadow-md hover:shadow-blue-500/30 hover:scale-105 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-blue-500"
                    >
                        <img
                            src={url}
